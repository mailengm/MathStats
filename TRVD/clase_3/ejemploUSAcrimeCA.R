...
# Lo que vale el autovector contra los datos. La proyección.
...
res.pca$center # La media
res.pca$scale # Desvío
...
res.pca$rotation # los autovectores que puede calcular (los que el rango permita)
...
head(res.pca$x) # Los datos en la base de componentes principales
...
res.pca$sdev^2/sum(res.pca$sdev^2) #% de varianza explicada por la componente


# Libreria factoextra R <- Tiene herramientas interesantes de visualizacion
...
# Convención: la primera componente de PCA tiene que ser positiva.
...
#de acuerdo con los 2 componentes plirincipales. 
...
## El de Florida
get_pca_ind(res.pca)$cos2[9,]

## los replica a mano para chequear
...
#$contrib contribuciones. No sabemos bien qué hace.
...
#######################
#La representaci?n de las variables difiere de la gr?fica 
#de las observaciones: las observaciones est?n representadas 
#por sus proyecciones, pero las variables est?n representadas
#por sus correlaciones

get_pca_var(res.pca)

#$coord: coordenadas de variables para crear un diagrama de dispersi?n
...
#$cos2: representa la calidad de representaci?n de las variables en el mapa de factores. Se calcula como las coordenadas al cuadrado: var.cos2 = var.coord * var.coord.
...
#$contrib: contiene las contribuciones (en porcentaje) de las variables a los componentes principales. La contribuci?n de una variable (var) a un componente principal dado es (en porcentaje) : (var.cos2 * 100) / (cos2 total del componente).
...
#La distancia entre las variables y el origen mide la calidad de las variables en el mapa de factores. Las variables que est?n alejadas del origen est?n bien representadas en el mapa de factores.
...
# PCA() -tiene PCA con resultados m?s detallados. Los valores ausentes se reemplazan por la media de cada columna. Pueden incluirse variables categ?ricas suplementarias. Estandariza autom?ticamente los datos.

#library(factoextra)

# get_pca()  Extrae la informaci?n sobre las observaciones y variables de un an?lisis PCA.
# get_pca_var() Extrae la informaci?n sobre las variables.
# get_pca_ind()  Extrae la informaci?n sobre las observaciones.

#Visualizaciones:
# library(FactoMineR)

# fviz_pca_ind() Representaci?n de observaciones sobre componentes principales.
# fviz_pca_var() Representaci?n de variables sobre componentes principales.
# fviz_contrib() Representa la contribuci?n de filas/columnas de los resultados de un pca.
# fviz_screeplot() -> Representaci?n (gr?fico barras) de eigenvalores.
